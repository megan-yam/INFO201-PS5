...
2. If you did this correctly, you saw that there are more iso-2 codes than names, and there are even more *iso3*-codes. What is going on? Can you find it out?
    ...
    data %>%
      group_by(iso3) %>%
      summarise(iso3code = n_distinct(name)) %>%
      arrange()
    ```
...
```{r}
  data %>%
    filter(!is.na(time)) %>%
    summarise(minimum = min(time), maximum = max(time))
```
---------------------------------------------------------------------
...
data %>%
  ...
  filter(!is.na(co2_PC), !is.na(region), !is.na(time), region != "") %>%
...
```{r}
data %>%
  filter(!is.na(name), !is.na(region), region != "", name != "", time == "2019"
         , !is.na(co2_PC)) %>%
